(function () {
  ...
  var _load = function () {
    if (!window.context) {
      console.log(
        "ThreePanesView FreshRSS extension waiting for FreshRSS to be initialized"
      );
    ...
    stream.insertAdjacentHTML(
      "beforebegin",
      `<div id="threepanesviewcontainer"></div>`
    );
    var wrapper = document.getElementById("threepanesviewcontainer");
    wrapper.appendChild(stream);
    wrapper.insertAdjacentHTML(
      "beforeend",
      `<div id="threepanesview"><div class="flux">${html}</div></div>`
    );
    ...
    var _resize = function () {
      ...
      if (topOffset > 500) window.setTimeout(_resize, 10);
      else {
        ...
        if (navEntries) availableHeight -= navEntries.clientHeight;
    ...
    var setContent = function (html, articleId) {
      ...
      //something else).
      if (!(wrapper.getAttribute("style") || "").includes("height")) _resize();
    ...
    var onArticleOpened = function (articleElement) {
      ...
      var articleContent =
        articleElement.querySelector(".flux_content").innerHTML;
